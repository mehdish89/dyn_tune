...
def restamp(bag, output_bag_file = 'restamped.bag', values = [], prefix = "", start_signal = {}, end_signal = {}):
	...
	try:
		for topic, msg, t in bag.read_messages():
			...
			if not any([ value.find(topic)>-1 for value in values]):
				continue
...
def interpobj(t, y):
	...
	# TODO: rememeber to double check the correctness of delegate function
	def f(t_new, t = t, y = y):
...
def start_simulation(real_bag_file, sim_bag_file, src_topic, dst_topic, values = [] , start_signal={}, end_signal={}):
	...
	sim_bag = restamp(sim_bag, output_bag_file = 'sim-restamped.bag' , values = values, prefix = SIM_PREFIX, start_signal = start_signal, end_signal = end_signal)
	real_bag = restamp(real_bag, output_bag_file = 'real-restamped.bag', values = values, prefix = REAL_PREFIX, start_signal = start_signal, end_signal = end_signal)
...
def resample(bag, value_list, resolution = 0.1):
	...
	for value in value_list:
		...
		if len(t)>1 and (dt == int or dt == float): # TODO: double check the correctness of type checking
...
class dynamic_tuner:
	...
	def __init__(   self, 
		...
		if type(self._param_set) == list:
			self._params_desc = self.get_params_desc(self._param_set)
		else:
			self._params_desc = self._param_set
